...
exports.getRetailerOrders = async(req, res) => {
    ...
    try {
      ...
      SELECT o.id AS orderId, o.userId, o.status, o.orderDate, o.address, o.phone,
             u.fullName AS customerName, p.id AS productId, p.name AS productName,
             p.photo AS productPhoto, o.quantity
      FROM orders o
...
exports.getCustomerOrders = async(req, res) => {
    ...
    try {
      ...
      SELECT o.id AS orderId, o.status, o.orderDate, o.address, o.phone,
             p.id AS productId, p.name AS productName, p.photo AS productPhoto, o.quantity
      FROM orders o
      JOIN products p ON p.id = o.productId
      WHERE o.userId = ?
      ORDER BY o.orderDate DESC
...
exports.deleteOrder = async(req, res) => {
    ...
    try {
        const [check] = await db.query(`SELECT id FROM orders WHERE id = ? AND userId = ?`, [orderId, userId]);
        if (check.length === 0) {
