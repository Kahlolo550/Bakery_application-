...
exports.addToCart = async(req, res) => {
    ...
    try {
        const [existing] = await db.query(
            'SELECT id FROM cart WHERE userId = ? AND productId = ?', [userId, productId]
        );

        if (existing.length > 0) {
            await db.query(
                'UPDATE cart SET quantity = quantity + ? WHERE userId = ? AND productId = ?', [quantity, userId, productId]
            );
        } else {
            await db.query(
...
exports.updateCartItem = async(req, res) => {
    ...
    try {
        await db.query(
            'UPDATE cart SET quantity = ? WHERE id = ? AND userId = ?', [quantity, cartId, userId]
        );
...
exports.checkoutCart = async(req, res) => {
    ...
    try {
        ...
        for (const item of cartItems) {
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [userId, item.productId, item.quantity, orderDate, status, address, phone, note]
            );
        }
